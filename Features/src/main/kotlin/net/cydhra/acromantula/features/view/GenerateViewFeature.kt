...
object GenerateViewFeature {
    ...
    fun getViewTypes(): List<Pair<String, String>> {
        return this.registeredGenerators.keys
            .map { type -> type to registeredGenerators[type]!!.fileType.fileExtension }
            .toList()
    }
    ...
    /**
     * Get the file extension generated by the given generator type or `null` if no such generator exists
     */
    fun getFileExtension(generatorType: String): String? {
        return this.registeredGenerators[generatorType]?.fileType?.fileExtension
    }
}